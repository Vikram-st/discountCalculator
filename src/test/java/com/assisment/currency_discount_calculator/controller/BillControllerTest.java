...
import com.assisment.currency_discount_calculator.model.*;
...
class BillControllerTest {
    ...
    void calculate_Success() {
        ...
        when(currencyExchangeService.getExchangeRates("USD")).thenReturn(Map.of("rates", exchangeRates));
...
//        verify(discountCalculationServices).calculateDiscount(user, 350, items);
    ...
    void calculate_InvalidCurrency() {
        ...
        when(currencyExchangeService.getExchangeRates("INVALID_CURRENCY")).thenThrow(new RuntimeException("Currency not found"));
        ...
        verify(currencyExchangeService).getExchangeRates("INVALID_CURRENCY");
